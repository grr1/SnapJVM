 ...
 */
void mallocTester::initialize_test(const char * name) {
  const char * filename = strrchr(name, '/');
  printf("TEST: %s\n", filename ? filename+1 : name);

  printf("INTIAL STATE\n\n");
  printf("FREELIST\n");
  mPrinter->freelist_print(mHeap, print_object);
  printf("TAGS\n");
  mPrinter->tags_print(mHeap, print_object);
}
 ...
 *        validity
 */
void mallocTester::finalize_test() {
  printf("FINAL STATE\n\n");
 ...
 */
void * mallocTester::malloc_and_clear(size_t size) {
  // Allocate memory
 ...
 */
void ** mallocTester::mallocing_loop(void ** array, size_t size, size_t n, printFormatter pf, bool silent) {
  if (!silent) {
    if (n == 1) {
 ...
 */
void * mallocTester::mallocing(size_t size, printFormatter pf, bool silent) {
  void * p;
 ...
 */
void mallocTester::check_and_free(char * p, size_t size) {
 ...
 */
void mallocTester::freeing_loop(void ** array, size_t size, size_t n, printFormatter pf, bool silent) {
  if (!silent) {
    if (n == 1) {
 ...
 * @param silent If true don't print anything
 */
void mallocTester::freeing(void * p, size_t size, printFormatter pf, bool silent) {
