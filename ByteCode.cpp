...
/* Derived from OpenJDK hotspot/share/interpreter/bytecodes.cpp
 */
...
bool          	ByteCode::_is_initialized;
const char*   	ByteCode::_name          [number_of_codes];
BasicType     	ByteCode::_result_type   [number_of_codes];
u1 				ByteCode::_depth         [number_of_codes];
u1            	ByteCode::_lengths       [number_of_codes];
ByteCode::Code  ByteCode::_java_code     [number_of_codes];
u2         		ByteCode::_flags         [(1<<8)*2]; // all second page for wide formats

void ByteCode::def(Code code, const char* name, const char* format, const char* wide_format, BasicType result_type, int depth, bool can_trap) {
	def(code, name, format, wide_format, result_type, depth, can_trap, code);
}

void ByteCode::def(Code code, const char* name, const char* format, const char* wide_format, BasicType result_type, int depth, bool can_trap, Code java_code)
{
	...
	_flags[(u1)code+0*(1<<8)] = compute_flags(format,      bc_flags);
	_flags[(u1)code+1*(1<<8)] = compute_flags(wide_format, bc_flags);
}
...
int ByteCode::compute_flags(const char* format, int more_flags) {
